...
const hasModel=(c,key)=>Boolean(c.models?.[key]?.url);
const hasTransform=(c,key)=>c.transforms?.[key]?.matrix?.length===16;
function availableStages(c){
 const has=key=>hasModel(c,key);
 return {color:has('post'),registration:has('pre')&&has('post')&&Object.keys(names).some(k=>hasTransform(c,k)),extraction:has('post')&&has('extracted'),cervical:has('extracted')&&has('corrected'),segmentation:has('segmented')&&(has('corrected')||has('extracted'))};
}
function executionNote(c){
 const status=typeof c.execution_status==='object'?c.execution_status?.status:c.execution_status;
 const reason=c.reason||c.execution_reason||c.execution_status?.reason;
 return [hasModel(c,'segmented')?'':'牙面分割未生成',status?'执行状态：'+status:'',reason?'原因：'+reason:''].filter(Boolean).join('；');
}
function configureStages(c){
 const available=availableStages(c);
 for(const button of $('stages').children){
  button.dataset.stageLabel||=button.textContent;button.disabled=!available[button.dataset.stage];button.setAttribute('aria-disabled',String(button.disabled));
  button.textContent=button.dataset.stageLabel+(button.disabled?'（未生成）':'');button.title=button.disabled?'此阶段产物未生成':'';button.style.opacity=button.disabled?'.5':'';button.style.cursor=button.disabled?'not-allowed':'';
  if(button.dataset.stage==='cervical')button.hidden=!available.cervical;
 }
 if(!available[stage])stage=['extraction','registration','color','cervical','segmentation'].find(k=>available[k])||null;
 for(const id of ['left-transform','right-transform']){const select=$(id);for(const option of select.options)option.disabled=!hasTransform(c,option.value);if(!hasTransform(c,select.value))select.value=Object.keys(names).find(k=>hasTransform(c,k))||'';}
 for(const option of $('color-source').options)option.disabled=!hasModel(c,option.value);
 if(!hasModel(c,$('color-source').value))$('color-source').value='post';
 for(const option of $('extraction-left').options)option.disabled=option.value==='difference'?!hasModel(c,'difference'):option.value==='color'?!hasModel(c,'color_target'):!hasModel(c,'post');
 if($('extraction-left').selectedOptions[0]?.disabled)$('extraction-left').value='raw';
 $('reference').disabled=!hasModel(c,'reference');$('context').disabled=!hasModel(c,'post');
}
function targetBounds(c){return c.target_bounds||c.models?.extracted?.bounds||c.models?.segmented?.bounds||c.models?.post?.bounds||c.models?.pre?.bounds;}
...
function geometry(m,mode,only=null){
 ...
 for(let i=0;i<m.nv;i++){
  ...
  else if(mode==='segmentation')c.set(DISPLAY_PALETTE[lab]||'#96a3b2');
...
function plane(){
 const b=targetBounds(C()),z=b[0][2]+(b[1][2]-b[0][2])*(Number($('clip-height').value)/100),s=$('clip-flip').checked?-1:1;
...
function bounds(){
 const c=C();let b;
 if($('scope').value==='arch'){b=box(c.models.pre?.bounds||targetBounds(c));if(c.models.post?.bounds)for(const k of stage==='registration'?[$('left-transform').value,$('right-transform').value]:['measurement']){const post=box(c.models.post.bounds);if(hasTransform(c,k))post.applyMatrix4(matrix(k));b.union(post)}}
 else if($('scope').value==='tooth')b=box(stage==='color'?((c.instances||[]).find(i=>i.id===Number($('instance').value))?.bounds||targetBounds(c)):targetBounds(c));
 else b=box(c.roi_bounds||targetBounds(c));
 return b;
}
function fit(){
...
async function render(doFit=false){
 const ticket=++serial,c=C();configureStages(c);$('fatal').textContent='';
 if(!stage){panels.forEach(clear);panels.forEach(draw);$('status').textContent='无可显示的阶段产物';$('stage-note').textContent=executionNote(c)||'此例尚未生成可显示的模型。';return;}
 $('status').textContent='正在加载模型…';
 // 只读取当前阶段实际使用的模型，缺少下游产物不能阻断已有配准或提取结果。
 const needed={registration:['pre','post'],color:['post',$('color-source').value,...($('color-evidence').checked?['color_omission_'+$('instance').value]:[])],extraction:['post','extracted',$('extraction-left').value==='color'?'color_target':$('extraction-left').value==='difference'?'difference':'post',...($('reference').checked?['reference']:[]),...($('non-source').checked?['extracted_non_source']:[])],cervical:['extracted','corrected',...($('removed-tissue').checked?['removed']:[])],segmentation:['corrected','extracted','segmented',...($('non-source').checked?['extracted_non_source','segmented_non_source']:[])]};
 const keys=[...new Set(needed[stage])].filter(k=>hasModel(c,k));
 ...
 document.querySelector('header h1').textContent=stage==='segmentation'?'最终牙面与肩台分割':'配准 → 单牙提取 → 牙面分区';
 document.querySelector('header p').textContent=stage==='segmentation'?'左侧核对牙体形状，右侧查看实际分区；可旋转检查颈缘与肩台交界。':'真实流水线中间结果 · 配准、选牙与分区分别检查';
 document.title=stage==='segmentation'?'最终牙面与肩台分割':'配准与单牙提取 · 逐阶段排查';
 $('metrics').hidden=stage==='segmentation';
 for(const button of $('cases').querySelectorAll('[data-case]')){
  const item=data.cases.find(row=>row.id===button.dataset.case),tag=button.querySelector('small');
  if(!item||!tag)continue;
  tag.textContent=stage==='segmentation'?(hasModel(item,'segmented')?'有分割':'未生成'):(item.gate?.['状态']||item.gate?.status||'不可用');
  tag.classList.toggle('fail',stage==='segmentation'?!hasModel(item,'segmented'):item.gate?.status==='FAIL');
 }
 if(stage==='registration'){
  ...
  if(c.coordinates==='raw_post')$('stage-note').textContent='尚未生成可用配准，当前为备牙后原扫描坐标；方向按钮只表示观察方向，不是确认后的解剖方向。左右分别显示真实原扫描与 Step1 颜色标签。开启“纯灰”可区分几何孔洞与标签空白；此处不展示配准或单牙提取结果。';
  const evidence=c.color_evidence?.[$('instance').value];if(evidence){$('stage-note').textContent+=' '+evidence.conclusion;if($('color-evidence').checked&&m['color_omission_'+$('instance').value])add(right,m['color_omission_'+$('instance').value],'#ff8a28',{order:3})}
  ...
  if($('reference').checked&&m.reference)for(const p of panels)add(p,m.reference,'#58d5e3',{wire:true,opacity:.5,order:2});
 ...
 if(doFit)fit();else panels.forEach(draw);$('status').textContent='已加载 · 原始网格分辨率'+(!hasModel(c,'segmented')?' · 牙面分割未生成':'');url();
}
...
function choose(id){
 ...
 const cu=data.comparison_url||'./?v=segmentation-r6-correct-target';
 $('title').textContent=c.name;$('old-view').href=cu+'#'+c.id+(cu.includes('stages.html')?'/shoulder':'');
 $('old-view').hidden=!hasModel(c,'segmented');
 if(data.comparison_url)$('old-view').textContent=data.comparison_name||'肩台候选对照';
 configureStages(c);
 $('selection').textContent=(c.color_target_recognized?'目标实例 '+c.post_instance:'颜色分牙未识别目标')+' · '+(c.selection_description||(c.coordinates==='raw_post'?'目标身份不可用，尚未完成配准与提取':c.selection_user_confirmed?'已按用户截图纠正':'沿用当前审阅目标，仍可核对'));
 ...
 $('metrics').replaceChildren();metric('局部配准状态',c.gate?.['状态']||c.gate?.status||'不可用');metric('稳定锚点原始 P95',mm(c.gate?.['留出锚点P95_mm']));
 if(c.gate?.['留出锚点P95_扣噪声底_mm']!=null)metric('门禁 P95（扣噪声底 '+mm(c.gate?.heldout_noise_floor_mm)+'）',mm(c.gate['留出锚点P95_扣噪声底_mm']));
 metric('候选配准在目标区的分歧',mm(c.gate?.['备牙ROI候选分歧P95_mm']));metric(c.color_target_recognized?'颜色目标未进入提取':'恢复候选未进入提取',c.qa?.color_vertices_lost_in_extraction==null?'不可用':c.qa.color_vertices_lost_in_extraction+' 顶点');
 ...
 $('provenance').textContent='PRE 内部目标标签 '+(c.pre_label??'不可用')+'；邻牙标签 '+(c.neighbor_labels?.join('、')||'不可用')+'。实际读取变换：'+(c.measurement_transform||'不可用')+'。'+(c.diagnostic_fallback||(c.coordinates==='raw_post'?'尚未生成可用配准或单牙提取结果。':'使用备牙前独立单牙范围提取。'))+' 单牙颜色与提取差异不等同于分割准确率。';$('audit').hidden=!c.audit_url;if(c.audit_url)$('audit').href=c.audit_url;
 if(c.registration_source)$('provenance').textContent+=' 配准来源：'+c.registration_source.source_branch+' @ '+c.registration_source.source_commit.slice(0,12)+'；执行状态以阶段记录为准。';
 render(true);
}
for(const c of data.cases){
 const b=document.createElement('button');b.className='case';b.dataset.case=c.id;b.append(document.createTextNode(c.name));const tag=document.createElement('small');tag.textContent=c.gate?.['状态']||c.gate?.status||'不可用';if(c.gate?.status==='FAIL')tag.className='fail';b.append(tag);b.onclick=()=>choose(c.id);$('cases').append(b);
 ...
 for(const val of [c.gate?.['状态']||c.gate?.status||'不可用',mm(c.gate?.['留出锚点P95_mm']),c.qa?.color_vertices_lost_in_extraction??'—',c.qa?.extraction_vertices_outside_color??'—',['color','extraction','segmentation'].map(k=>c.qa?.topology?.[k]?.boundary_components??'—').join(' / ')]){const td=document.createElement('td');td.textContent=val;tr.append(td)}$('summary').append(tr);
}
for(const b of $('stages').children)b.onclick=()=>{if(b.disabled)return;stage=b.dataset.stage;if(stage==='registration')$('scope').value='neighbors';else $('scope').value=stage==='color'?'neighbors':'tooth';render(true)};
